(ns top.kzre.krro.plugin.painting.platform.javafx.input
  "JavaFX 平台输入源实现。将 JavaFX Canvas 上的鼠标事件转换为标准事件。"
  (:require [top.kzre.krro.plugin.painting.canvas.input :as input]
            [top.kzre.krro.plugin.painting.canvas.event :as event])
